 ...
 * @param {Buffer} buffer Binary to disassemble.
 * @param {Number} addr Starting address for the given buffer.
 * @param {Boolean function(CsInsn)} callback Function returning false will invalidate/reset the iterator. disasm_iter
 * will then return false.
 * It is useful in scenario of progressive disassembling seeking for a particular instruction.
 * @return {Boolean} True if more instructions are available,otherwise the iteration is complete, 
 * i.e. the callback has returned false.
 */
exports.Cs.prototype.disasm_iter = function (buffer, addr, callback) {
    ...
    if(this.hasOwnProperty("_iter_stop")){
        return false;
    }
