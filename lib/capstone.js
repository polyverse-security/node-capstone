 ...
 * @param {Buffer} buffer Binary to disassemble.
 * @param {Number} addr Starting address for the given buffer.
 * @param {CsInsn} insn Disassembled instruction detail.
 * @return {Boolean} True if successfully disassembled one instruction.
 */
exports.Cs.prototype.disasm_iter = function (buffer, addr, callback) { //(iter, callback) {
    // TODO
...
//            ] ],
    ...
    if(success){
        ...
        if(callback) {
            if(!callback(ret)){
                delete this._iter_code_ref;
                delete this._iter_code_size_ref;
                delete this._iter_address_ref;
                delete this._iter_insn;
            }
        }
        console.log("Cs.prototype.disasm_iter returning: ", ret);
        return ret;
    }
    return null;
};
