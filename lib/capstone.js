...
if(/^win/.test(process.platform)) {
  var libName = "capstone";
} else {
  var libName = "libcapstone";
}
 ...
 * - `ERR_X86_ATT`: X86 AT&T syntax is unsupported
 * - `ERR_X86_INTEL`: X86 Intel syntax is unsupported
 * @name ERR
 * @enum
 */
exports.ERR_OK          = 0;
...
exports.ERR_X86_ATT     = 12;
exports.ERR_X86_INTEL   = 13;
...
exports.MODE_MCLASS         = 1 << 5;
exports.MODE_MICRO          = 1 << 4;
exports.MODE_MIPS3          = 1 << 5;
exports.MODE_MIPS32R6       = 1 << 6;
exports.MODE_MIPSGP64       = 1 << 7;
exports.MODE_V9             = 1 << 4;
 ...
 *             offset.toString(16),
 *             code.toJSON().data.map(function (value) {
 ...
 */
try {
    var capstone = ffi.Library(libName, {
        ...
        "cs_disasm_iter": [ "bool", [
            "size_t", "uint8 **", "size_t *", "uint64 *", ref.refType(InsnType)
        ] ],
        "cs_malloc"     : [ ref.refType(InsnType), [ "size_t" ] ],
 ...
 * @return {Boolean} True if Capstone supports the given arch or support.
 */
 ...
 * @param {Buffer} buffer Binary to disassemble.
 * @param {Number} addr Starting address for the given buffer.
 * @param {Boolean function(CsInsn)} callback Function returning false will invalidate/reset the iterator.
 * It is useful in scenario of progressive disassembling seeking for a particular instruction.
 * @return {Boolean} True if successfully disassembled one instruction.
 */
exports.Cs.prototype.disasm_iter = function (buffer, addr, callback) {
    // TODO
    // `iter` is a dictionary that the iterator will update
    // `callback` is called for each iteration, receiving the CsInsn
    // Will need to internally allocate a cs_insn buffer for this method

    // console.log("Cs.prototype.disasm_iter this: ", JSON.stringify(this));
    // console.log("Cs.prototype.disasm_iter buffer: ", JSON.stringify(buffer));
    // console.log("Cs.prototype.disasm_iter addr: 0x", addr.toString(16));

    if(!callback) {
        console.log("Cs.prototype.disasm_iter callback missing: ");
        return false;
    }
    var cshandle = this.csh.deref();
    // console.log("Cs.prototype.disasm_iter cshandle: ", cshandle);

    if(!this.hasOwnProperty("_iter_code_ref")){
        this._iter_code_ref = buffer.ref();
    }
    // console.log("Cs.prototype.disasm_iter this._iter_code_ref: ", this._iter_code_ref);

    if(!this.hasOwnProperty("_iter_code_size_ref")){
        this._iter_code_size_ref = ref.alloc(ref.types.size_t, buffer.length);
    }
    // console.log("Cs.prototype.disasm_iter this._iter_code_size_ref: ", this._iter_code_size_ref);

    if(!this.hasOwnProperty("_iter_address_ref")){
        this._iter_address_ref = ref.alloc(ref.types.long, addr);
    }
    // console.log("Cs.prototype.disasm_iter this._iter_address_ref: ", this._iter_address_ref);

    if(!this.hasOwnProperty("_iter_insn")){
        this._iter_insn = capstone.cs_malloc(cshandle);
    }
    // console.log("Cs.prototype.disasm_iter this._iter_insn: ", this._iter_insn);

    var invalidate_iter = function(iter){
        delete iter._iter_code_ref;
        delete iter._iter_code_size_ref;
        delete iter._iter_address_ref;
        delete iter._iter_insn;
    };
    var success;
    try {
//            "cs_disasm_iter": [ "bool", [
//                "size_t", "uint8 **", "size_t *", "uint64 *", ref.refType(InsnType)
//            ] ],
    console.log("Cs.prototype.disasm_iter this._iter_code: " + this._iter_code_ref.deref().toString());
    console.log("Cs.prototype.disasm_iter this._iter_code_size: 0x" + this._iter_code_size_ref.deref().toString(16));
    console.log("Cs.prototype.disasm_iter this._iter_address_ref: 0x" + this._iter_address_ref.deref().toString(16));
    success = capstone.cs_disasm_iter(
        cshandle,
        this._iter_code_ref,
        this._iter_code_size_ref,
        this._iter_address_ref,
        this._iter_insn);
    } catch (ex) {
        console.log("Cs.prototype.disasm_iter exception: ", ex);
        return false;
    }

    console.log("Cs.prototype.disasm_iter success: ", success);
    if(success){
        var insns = this._iter_insn.deref();
        var ret = new CsInsn(insns, this.arch);
        if(callback) {
            if(!callback(ret)){
                invalidate_iter(this);
                success = false;
            }
        }
    }
 ...
 * @return {Boolean} True if the instruction belongs to the given group.
 */
 ...
 * @param {CsInsn} insn Instruction
 * @param {Number} id Register ID
 * @return {Boolean} True if the instruction reads the given register.
 */
 ...
 * @param {CsInsn} insn Instruction
 * @param {Number} id Register ID
 * @return {Boolean} True if the instruction writes the given register.
 */
